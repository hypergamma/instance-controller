...
router.put('/control/function', function(req, res, next) {
    ...
    /* 3. 신규 function docker image 생성 및 private docker repo 에 push */
    ...
    var observer_imagenae = getObserverImageName(nuser, nfunc);
    var observer_dockerpath = _dockerroot + "observer/";
    ...
    async.waterfall([
            ...
            var swarm = exec(_actionroot + "swarm/swarm-create-service.sh"
                ...
                , function (error, stdout, stderr) {
                    if (error !== null) {
                        ...
                        result.message = "observer swarm 서비스 생성 성공";
                    }
        ...
        function(result, callback) {
            /* swarm endpoint registry 적재  */
            var endpoint = "";
            registry.setFunctionEndpoint(nuser, nfunc, endpoint, function(err, res) {
                if (err) {
                    result.message = "swarm endpoint 적재 실패";
                    result.error = err;
                } else {
                    result.message = "swarm endpoint 적재 성공";
                }
...
router.delete('/control/function', function(req, res, next) {
    ...
    async.waterfall([
        ...
        /* 2. registry 에서 swarm endpoint 제거 */
        function(result, callback) {
            registry.delFunctionEndpoint(nuser, nfunc, function(err, res) {
                if (err) {
                    result.message = "swarm endpoint registry 삭제 실패";
                    result.error = err;
                } else {
                    result.message = "swarm endpoint registry 제거 성공";
                }
