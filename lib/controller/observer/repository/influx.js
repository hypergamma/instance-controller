...
function getResourceUsage(second, resource, result) {

    var tnow = moment().utc().subtract(second, 'seconds');
    var tnow_str = tnow.format();

    var query = `select mean(${resource}) from handler where nuser = '${nuser}' and nfunc = '${nfunc}' and time > '${tnow_str}' group by time(${second}s)`;
