...
function adjustFunctionReplica(replica, result) {
    var req_body = { nfunc: nfunc, nuser: nuser, replica: replica };
    client.post('/api/control/function', req_body, (err, req, res, obj) => {
        if (err) result(err);
        else result(obj);
    });
}
...
function getFunctionReplica(result) {
    ...
    client.post('/api/status/function/replica', req_body, (err, req, res, obj) => {
        if (err) {
            result(err);
        } else {
            result(obj);
